...
public class Server {
    ...
    public static File commandIDsFile = new File("logs/commandIDs.txt");

    //файлы для хранения информации о пользователях
    public static File connectionsFile = new File("logs/connectionsFile.txt");
    public static File poweringFile = new File("logs/on or off.txt");
    ...
    public static void createFiles() {
        try {
            ...
            printColored("Creating logs dir: " + logs.mkdir() + "\n", FILECREATINGCOLOR);
            printColored("Logs folder path: " + logs.getAbsolutePath() + "\n", LOGCOLOR);
    ...
    public static void fillArrays() {
        ...
        if (idSplit[0].equals("")) {
            printColored("No id's to parse", INVALIDDATACOLOR);
            return;
        }
    ...
    public static void setIdCount() {
        ...
        if (ids.trim().equals("")) {
            ...
            for (int i = 0; i < idSplit.length; i++) {
                addReplace(commandIds, Integer.parseInt(idSplit[i].trim()));
            }
            if (commandIds.size() > 0) {
                Request.setCount(commandIds.get(commandIds.size() - 1));
            } else {
                Request.setCount(1);
            }
        }
                    ...
                    if (action.matches("/disconnect[ ]*\\d*[ ]*")) { // - /disconnect [int id]
                        if (action.split("/disconnect").length > 0) {
                            ...
                            if (activeIds.contains(idToDisconnect)) {
                                writeSuccess = getPhoneById(phones, idToDisconnect).writeLine("SYS$DISCONNECT");
                                checkWriteSuccess(writeSuccess, getPhoneById(phones, idToDisconnect), Thread.currentThread());
                            ...
                            for (int i = 0; i < phones.size(); i++) {
                                writeSuccess = phones.get(i).writeLine("SYS$DISCONNECT");
                                checkWriteSuccess(writeSuccess, phones.get(i), Thread.currentThread());
                                writeConnection(Math.abs(phones.get(i).id), 'd');
                            }
                        ...
                        if (action.split("/msg").length > 0) {
                            ...
                            for (int i = 2; i < action.split(" ").length; i++) {
                                messageText += action.split(" ")[i];
                            }

                            refreshActiveIDs();//обновление итендификаторов перед отправкой, иначе воможна отправка несуществующему клиенту

                            if (activeIds.contains(idToSend)) {
                                writeSuccess = getPhoneById(phones, idToSend).writeLine("SYS$MSG$" + messageText);
                                checkWriteSuccess(writeSuccess, getPhoneById(phones, idToSend), Thread.currentThread());
    ...
    //добавление с перезаписью для типа int (для Id)
        ...
        try (ServerSocket server = new ServerSocket(PORT)) { //запуск сервера
            ...
            while (true) {
                ...
                new Thread(() -> {//каждый клиент в отдельном потоке
                    ...
                    String data, root = "", command, args, success, dataSent;
                    ...
                    int uniId = 0, aUniId, cUniId, writeSuccess;
                    ...
                    do {//пока клиент не зарегается или не войдёт
                        ...
                        checkReadSuccess(dataSent, phone);
                        if (dataSent.equals("-2")) { //обработка отключения во время входа
                            isActive = false;
                            printColored("Interrupting client working thread", LOGCOLOR);
                            ...
                            if (uniId <= 0) {
                                if (idAll.contains(-uniId)) {
                                    ...
                                    writeSuccess = phone.writeLine("Invalid id: user with id " + (-uniId) + " already exists");
                                    checkWriteSuccess(writeSuccess, phone, Thread.currentThread());
                                    continue;
                                }
                                ...
                                writeSuccess = phone.writeLine("Registration success");
                                checkWriteSuccess(writeSuccess, phone, Thread.currentThread());

                                phone.id = -uniId;
                                addReplace(idAll, Math.abs(uniId));//добавление нашего id в список
                                ...
                                if (idAll.contains(uniId)) {
                                    if (!activeIds.contains(uniId)) { //id есть в списке, но нет онлайн
                                        flag = false;
                                        writeSuccess = phone.writeLine("Login success");
                                        checkWriteSuccess(writeSuccess, phone, Thread.currentThread());
                                        phone.id = uniId;
                                        ...
                                        writeSuccess = phone.writeLine("Login failed: user with id " + uniId + " has already logged in");
                                        checkWriteSuccess(writeSuccess, phone, Thread.currentThread());
                                    }
                                    ...
                                    writeSuccess = phone.writeLine("Login failed");
                                    checkWriteSuccess(writeSuccess, phone, Thread.currentThread());
                                    flag = true;
                                }
                            }
                    ...
                    if (isActive) {
                        ...
                        phone.connection = "Ip: " + phone.getIp() + " id: " + uniId + " root: " + root;
                        printColored("Client connected: ip address is " + phone.getIp() + ", root is " + root + ", unique id is " + Math.abs(uniId), CONNECTIONCOLOR);
                        writeSuccess = phone.writeLine("Connected to server as " + root + " with unique id " + Math.abs(uniId));
                        checkWriteSuccess(writeSuccess, phone, Thread.currentThread());
                        writeConnection(Math.abs(uniId), 'c');
                    }
                    ...
                    while (isActive) {
                        ...
                        if (data != null) {
                            ...
                            if (split.length == 0) {
                                printColored("Received invalid data from client with id " + uniId, INVALIDDATACOLOR);
                                writeSuccess = phone.writeLine("INVALID SYNTAX");
                                checkWriteSuccess(writeSuccess, phone, Thread.currentThread());
                                continue;
                            }
                            ...
                            if (root.trim().equals("A")) {
                                ...
                                if (split[1].equals("getinfo")) {
                                    ...
                                    if (cUniId == phone.id) { //проверка на отправку запроса самому себе
                                        ...
                                        writeSuccess = phone.writeLine("INVALID ID SENT");
                                        checkWriteSuccess(writeSuccess, phone, Thread.currentThread());
                                        continue;
                                    }

                                    if (idA.contains(cUniId)) {//проверка на отправку запроса другому админу
                                        printColored("Attempt to send request to admin with id: " + idA.get(idA.indexOf(cUniId)), INVALIDDATACOLOR);
                                        writeSuccess = phone.writeLine("INVALID ID SENT");
                                        checkWriteSuccess(writeSuccess, phone, Thread.currentThread());
                                        continue;
                                    }
                                    ...
                                    printColored("Id who sent: " + uniId, LOGCOLOR);
                                    ...
                                    Request thisReq = new Request(aUniId, cUniId, command, args);
                                    ...
                                    if (toSend != null) {
                                        if (idAll.contains(cUniId)) {
                                            writeSuccess = toSend.writeLine(thisReq.id + "$" + command + "$" + args);
                                            checkWriteSuccess(writeSuccess, phone, Thread.currentThread());
                                        ...
                                        writeSuccess = phone.writeLine("Sending error: system didn't find an online client with id " + cUniId);
                                        checkWriteSuccess(writeSuccess, phone, Thread.currentThread());
                                    }
                                }
                                ...
                                printColored("Client id to send: " + uniId, LOGCOLOR);

                                //получение id админа, отправившего команду
                                aUniId = clientReq.idA;
                                addReplace(idA, aUniId);
                                printColored("Admin id to send: " + aUniId, LOGCOLOR);
                                ...
                                Phone toSend = getPhoneById(phones, aUniId);
                                if (toSend != null) {
                                    if (idAll.contains(aUniId)) {
                                        writeSuccess = toSend.writeLine(response);
                                        checkWriteSuccess(writeSuccess, phone, Thread.currentThread());
                                    } else { //ошибка отправки данных незарегистрированному администратору
                                        printColored("Invalid command: this id is free", INVALIDDATACOLOR);
                                    }
                                } else { //ошибка отправки данных оффлайн администратору
                                    printColored("Sending error: system didn't find an online admin with id " + aUniId, ERRORCOLOR);
                                    writeSuccess = phone.writeLine("\nSending error: system didn't find an online admin with id " + aUniId);
                                    checkWriteSuccess(writeSuccess, phone, Thread.currentThread());
                                }
                            }
    ...
    public static void getInfo(String command, Phone phone) {
        ...
        new Thread(() -> {
            ...
            switch (command.toLowerCase(Locale.ROOT)) {
                case "idall" -> toSend = "INFO$idAll" + activeIds.toString();
                case "idreg" -> toSend = "INFO$idReg" + idAll.toString();
                case "idA" -> toSend = "INFO$idA" + idA.toString();
                case "idC" -> toSend = "INFO$idC" + idC.toString();
                default -> {
                    if (command.matches("[\\d]+")) { //получение ip по id
                        ...
                        ArrayList<String> sucesss = new ArrayList<>();
                        ...
                        for (int i = 0; i < lines.length; i++) {
                            commands.add(lines[i].split("\\$")[3]);
                            args.add(lines[i].split("\\$")[4]);
                            sucesss.add(lines[i].split("\\$")[5]);
                        }
            ...
            writeSuccess = phone.writeLine(toSend);
            checkWriteSuccess(writeSuccess, phone, Thread.currentThread());
        }).start();
    }
    ...
    public static void updateIdCommandsFile(@NotNull Request req) {
